package com.honeybeedev.exclusiveprison.api.util.config.wrappers;
...
public abstract class ValuableWrapper {
    ...
    public boolean sectionIsPresent(String path) {
    ...
    public void ensureHasAll(String ...keys) {
        for (String key : keys) {
            if (!valueIsPresent(key) && !sectionIsPresent(key))
                throwError("Required key: " + key + " is not found!");
        }
    }

    public <T> void ifValuePresent(String path, Class<T> type, Consumer<T> consumer) {
        try {
            get(path, type).ifPresent(consumer);
        } catch (Throwable throwable) {}
    }

    public <T> void ifListValuePresent(String path, Class<T> type, Consumer<Collection<T>> consumer) {
        try {
            consumer.accept(getAsCollOf(path, type));
        } catch (Throwable throwable) {}
    }
}
