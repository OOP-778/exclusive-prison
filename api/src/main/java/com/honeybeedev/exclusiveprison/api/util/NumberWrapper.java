...
public class NumberWrapper {
    ...
    private static class Helper {
        ...
        private static final Function<String, Number>[] non_decimal_parsers = new Function[]{
        ...
        private static final Function<String, Number>[] decimal_parsers = new Function[]{
                in -> Floats.tryParse(String.valueOf(in)),
                in -> Doubles.tryParse(String.valueOf(in)),
                in -> new BigDecimal(String.valueOf(in))
        };

        private static final Map<Integer, Function<String, Number>> non_decimal_normalizers = new HashMap<>();
        private static final Map<Integer, Function<String, Number>> decimal_normalizers = new HashMap<>();
        ...
        public static String format(boolean suffixes, Number passed) {
            if (!suffixes)
                return current_format.format(passed);
...
//            }
        ...
        public static Number tryParse(String input) {
            try {
                ...
                if (StringUtils.contains(".", input))
                    return useParsers(decimal_parsers, input);
                else
                    return useParsers(non_decimal_parsers, input);
        ...
        public static Number normalize(Number number) {
            ...
            if (!format.contains(".")) {
                for (Map.Entry<Integer, Function<String, Number>> normalizerEntry : non_decimal_normalizers.entrySet()) {
            ...
            for (Map.Entry<Integer, Function<String, Number>> normalizerEntry : decimal_normalizers.entrySet()) {
                if (normalizerEntry.getKey() <= length) {
                    try {
                        Number parsed = normalizerEntry.getValue().apply(format);
                        if (parsed != null)
